import { FATWA_NS, trackFor } from './fatwas.mjs'
import {
  hasFile,
  markStoredByFile,
  openFileSink,
...
export function isStored(fileName) {
  return hasFile(FATWA_NS, fileName)
}
...
async function runTask(task) {
  ...
  let start = 0
  try {
    sink = await openFileSink(FATWA_NS, task.fileName, { url: task.url })
    start = sink.offset
    ...
    // كتابة الجسر صغيرة — FileWriter ينقل البيانات بصيغة base64.
    ...
    await sink.done()
    markStoredByFile(FATWA_NS, task.fileName, received - start)
...
/** يبدأ تحميل صوتية فتوى واحدة للاستماع دون إنترنت. */
export function downloadFatwa(fatwa, categoryName) {
  const file = trackFor(fatwa, categoryName)
...
/** إبطال مهمة جارية أو قيد الانتظار لصوتية معينة. */
export async function cancelRef(ref) {
  ...
  if (purgePartial) {
    await sleep(280)
    await removeFileBy(FATWA_NS, task.fileName)
  }
}

/** حذف صوتية محفوظة من الجهاز. */
export async function removeAudio(ref, fileName) {
  const task = tasks.get(ref)
  if (task?.state === 'running' && task.controller) task.controller.abort()
  if (task) {
    task.state = 'idle'
  ...
  if (fileName || task?.fileName) {
    await removeFileBy(FATWA_NS, fileName || task.fileName)
  }
  markDirty()
}
